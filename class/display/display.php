<?php
	...
	*/
	class display
	{
		...
		{
			global $conn;
			$sql = $conn->prepare('SELECT *, COUNT(id) as total FROM reservation WHERE status = ? GROUP BY id');
			$sql->execute(array(0));
			$count = $sql->rowCount();
			$fetch = $sql->fetch();
		...
		{
			global $conn;
			$sql = $conn->prepare('SELECT *, COUNT(id) as total FROM member WHERE m_status = ? GROUP BY id');
			$sql->execute(array(1));
			$count = $sql->rowCount();
			$fetch = $sql->fetch();
